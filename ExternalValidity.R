...
Merged=cbind(Data2[,1],Data1[,1])
for(i in 2:length(Covs)){Merged=cbind(Merged,Data2[,i],Data1[,i])}

Names2=rep(c("Population","Sample"),length(Covs))
...
p2 = ggplot(Merged, aes(Variable,Value)) + geom_boxplot(fill=rep(c(Population_Color,Sample_Color),length(Covs))) + coord_flip() + ylab(YLab)+ xlab(XLab)  + theme_bw() +theme(axis.title=element_text(size=10)) + ggtitle(Title) +theme(plot.title = element_text(lineheight=1.8,size=rel(1.5),face="bold"))	
