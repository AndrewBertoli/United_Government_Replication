...
, Poly.Order=NULL, Cluster = NULL, Jitter = FALSE, Loc.p.value = "BR",cex.main=1,cex.lab=1,
LabelRawData=FALSE, Labels, RawDataLabelSize=1, ...) 
{
    ...
    if (Plot.Means == FALSE) {
        plot(X[X < C], Y[X < C], xlab = xlab, ylab = ylab, main = Main, 
            ...
            cex = pointsize[midpoints < C], xaxt = "n", cex.main=cex.main,cex.lab=cex.lab,...)
...
if(LabelRawData==TRUE){
	
Labels=Labels[Y != 0]
X=X[Y != 0]
Y=as.numeric(Y[Y != 0])

Labels=Labels[X > -Bandwidth & X < Bandwidth]
Y=Y[X > -Bandwidth & X < Bandwidth]
X=X[X > -Bandwidth & X < Bandwidth]
	
    lay <- wordlayout(X, Y, Labels, cex=RawDataLabelSize, ...)
        for (i in 1:length(X)) {
            xl <- lay[i, 1] 
            yl <- lay[i, 2]
            w <- lay[i, 3]
...
# RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGG3YearsBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Main="3 Years Before Qualification", xlab="Points Above/Below Cut-Point",ylab="Militarized Interstate Disputes Initiated",Bandwidth=2.85621, Window=c(-2.5,2.5))

# RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGG3YearsAfter,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Main="3 Years After Qualification",xlab="Points Above/Below Cut-Point", ylab="", Bandwidth=3.2316, Window=c(-2.5,2.5))

# RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGG3YearsAfter-sample$AGG3YearsBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Main="Change in Aggression", xlab="Points Above/Below Cut-Point", ylab="", Bandwidth=4.1937, Window=c(-2.5,2.5))

# title("Figure 3. Change in Aggression for the World Cup",outer=TRUE,cex.main=2)
